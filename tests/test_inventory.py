...
def test_list_products_enhanced(client, owner_headers, test_product):
    ...
    assert len(data) >= 1
    # Check if our test product is in the list with correct values
    p_data = next((p for p in data if p["product_id"] == test_product.product_id), None)
    assert p_data is not None
    assert p_data["name"] == test_product.name
    assert float(p_data["selling_price"]) == float(test_product.selling_price)
...
def test_list_products_category_filter(client, owner_headers, test_product, test_store):
    resp = client.get(f"/api/v1/inventory/products?category_id={test_product.category_id}", headers=owner_headers)
    assert resp.status_code == 200
...
def test_get_product_enhanced(client, owner_headers, test_product):
    resp = client.get(f"/api/v1/inventory/products/{test_product.product_id}", headers=owner_headers)
    assert resp.status_code == 200
    data = resp.get_json()["data"]
    assert data["name"] == test_product.name
    assert data["sku_code"] == test_product.barcode  # Assuming barcode maps to sku_code in response
...
def test_soft_delete_product(client, owner_headers, test_product):
    resp = client.delete(f"/api/v1/inventory/products/{test_product.product_id}", headers=owner_headers)
    assert resp.status_code == 200
...
def test_stock_update_increments_stock(client, owner_headers, test_product):
    ...
    adj = db.session.query(StockAdjustment).filter_by(product_id=test_product.product_id).first()
...
def test_stock_update_with_cost_price_update(client, owner_headers, test_product):
    ...
    hist = (
        ...
        .filter_by(product_id=test_product.product_id)
...
def test_stock_audit_creates_records(client, owner_headers, test_product):
    ...
    payload = {
        "items": [{"product_id": test_product.product_id, "actual_qty": actual_qty}],
        "notes": "Monthly audit",
    }
    resp = client.post(
    ...
    audit_item = (
        db.session.query(StockAuditItem)
        .filter_by(
