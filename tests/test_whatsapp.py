...
def test_webhook_invalid_token_rejected(client):
    ...
    resp = client.get(
        "/api/v1/whatsapp/webhook?hub.mode=subscribe&hub.verify_token=wrong_token&hub.challenge=123"
    )
...
def test_send_alert_dry_run(client, owner_headers, wa_config, test_alert):
    ...
    resp = client.post(
        "/api/v1/whatsapp/send-alert",
        headers=owner_headers,
        json={"alert_id": test_alert.alert_id}
    )
...
def test_send_po_creates_log_entry(client, owner_headers, wa_config, test_po):
    ...
    resp = client.post(
        "/api/v1/whatsapp/send-po",
        headers=owner_headers,
        json={"po_id": str(test_po.id)}
    )
    assert resp.status_code == 200
    assert "message_id" in resp.json['data']
