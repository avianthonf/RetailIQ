...
def _seed_70_days(session, store_id):
    ...
    for i in range(70):
        ...
        pft = 40 + dow * 5
        session.add(DailyStoreSummary(
            store_id=store_id, date=d, revenue=rev, profit=pft, transaction_count=10
        ))
    session.commit()
...
def test_phase_2_prophet_vs_fallback(app, test_store, test_product):
    with app.app_context():
        ...
        for i in range(70):
            ...
            _db.session.add(DailySkuSummary(
                store_id=test_store.store_id, product_id=test_product.product_id,
    ...
    with app.app_context():
        ...
        _db.session.query(DailySkuSummary).filter(
