...
from app.models.expansion_models import Country, CountryTaxConfig, KYCProvider, PaymentProvider, StoreTaxRegistration
from app.payments.engine import get_payment_adapter
...
def expansion_setup(app, test_store):
    ...
    # Countries
    c_in = Country(code="IN", name="India", default_currency="INR", default_locale="en-IN", timezone="Asia/Kolkata", tax_system="GST", is_active=True)
    c_us = Country(code="US", name="United States", default_currency="USD", default_locale="en-US", timezone="America/New_York", tax_system="SALES_TAX", is_active=True)
    c_br = Country(code="BR", name="Brazil", default_currency="BRL", default_locale="pt-BR", timezone="America/Sao_Paulo", tax_system="IVA", is_active=True)
    ...
    t_us = CountryTaxConfig(country_code="US", tax_type="SALES_TAX", standard_rate=6.0, has_subnational_tax=True, subnational_config={"CA": 2.5, "NY": 4.0})
    t_br = CountryTaxConfig(country_code="BR", tax_type="IVA", standard_rate=17.0, e_invoice_required=True, e_invoice_format="NF_E")

    db.session.add_all([t_in, t_us, t_br])
    db.session.flush()

    # Store Tax Registrations
    reg_us = StoreTaxRegistration(store_id=test_store.store_id, country_code="US", tax_id="12-3456789", state_province="CA", is_tax_enabled=True)
    reg_br = StoreTaxRegistration(store_id=test_store.store_id, country_code="BR", tax_id="00.000.000/0001-91", is_tax_enabled=True)

    db.session.add_all([reg_us, reg_br])

    # Providers
    p_rzp = PaymentProvider(code="razorpay", name="Razorpay", country_code="IN", provider_type="PSP")
    p_str = PaymentProvider(code="stripe", name="Stripe", country_code="US", provider_type="PSP")
    p_pix = PaymentProvider(code="pix", name="PIX", country_code="BR", provider_type="BANK")
    ...
    db.session.add_all([p_rzp, p_str, p_pix, k_aadhaar, k_ssn])
    db.session.commit()
...
def test_payment_adapters(app, test_store, expansion_setup):
    """Test payment adapters."""
    rzp = get_payment_adapter("razorpay", test_store.store_id)
    intent = rzp.create_payment_intent(100.0, "INR", "test1234")
    assert intent["provider"] == "razorpay"
    assert intent["amount"] == 10000  # paise

    stripe = get_payment_adapter("stripe", test_store.store_id)
    intent2 = stripe.create_payment_intent(50.0, "USD", "test5678")
    assert intent2["provider"] == "stripe"
    assert intent2["amount"] == 5000  # cents
