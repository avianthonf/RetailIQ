...
def list_events():
    ...
    if from_date_str:
        try:
            ...
            return format_response(error={"code": "INVALID_DATE", "message": "Invalid from date format"}), 400
    ...
    if to_date_str:
        try:
            ...
            return format_response(error={"code": "INVALID_DATE", "message": "Invalid to date format"}), 400
...
def create_event():
    ...
    if not all(k in payload for k in required_fields):
        return format_response(error={"code": "VALIDATION_ERROR", "message": "Missing required fields"}), 400

    try:
        start_date = datetime.strptime(payload['start_date'], '%Y-%m-%d').date()
        end_date = datetime.strptime(payload['end_date'], '%Y-%m-%d').date()
    except ValueError:
        return format_response(error={"code": "INVALID_DATE", "message": "Invalid date format, use YYYY-MM-DD"}), 400

    if start_date > end_date:
        return format_response(error={"code": "VALIDATION_ERROR", "message": "start_date must be <= end_date"}), 400

    event_type = payload['event_type']
    if event_type not in {'HOLIDAY', 'FESTIVAL', 'PROMOTION', 'SALE_DAY', 'CLOSURE'}:
        return format_response(error={"code": "VALIDATION_ERROR", "message": "Invalid event_type"}), 400
...
def upcoming_events():
    ...
    try:
        ...
        return format_response(error={"code": "VALIDATION_ERROR", "message": "days must be an integer"}), 400

    today = datetime.now(timezone.utc).date()
    horizon_date = today + timedelta(days=days)

    events = db.session.query(BusinessEvent).filter(
        BusinessEvent.store_id == store_id,
        BusinessEvent.end_date >= today,
        BusinessEvent.start_date <= horizon_date
    ).order_by(BusinessEvent.start_date.asc()).all()
