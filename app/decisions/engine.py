...
def _dedup_and_sort(actions: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    unique_actions.sort(
        key=lambda x: (x.get("time_sensitive", False), x.get("priority", 0), x.get("confidence", 0.0)), reverse=True
    )
