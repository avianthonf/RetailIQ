...
def evaluate_alerts(self, store_id: int):
    ...
    with _RedisLock(lock_key, ttl=600) as acquired:
        ...
        with task_session() as session:
            ...
            # b) MARGIN_WARNING
            margin_products = session.execute(
                text("""
            ...
            """),
                {"store_id": store_id, "start": str(ma_start), "end": str(ma_end)},
            ).fetchone()

            today_row = session.execute(
                text("SELECT revenue FROM daily_store_summary WHERE store_id = :store_id AND date = :today"),
                {"store_id": store_id, "today": str(today)},
            ).fetchone()
...
def _get_top_skus(session, store_id: int) -> list[int]:
    ...
    """),
        {"sid": store_id, "window": PARETO_WINDOW_DAYS},
    ).fetchall()
    ...
    with _RedisLock(lock_key, ttl=3600) as acquired:
        ...
        with task_session() as session:
            ...
            for s in stores:
                ...
                """),
                    {"sid": s.store_id, "start": str(week_start), "today": str(today)},
                ).fetchone()
...
@shared_task(
    bind=True,
    name="tasks.check_overdue_purchase_orders",
    max_retries=3,
    default_retry_delay=120,
    autoretry_for=(Exception,),
    retry_backoff=True,
...
@shared_task(
    bind=True,
    name="tasks.auto_close_open_sessions",
    max_retries=3,
    default_retry_delay=120,
    autoretry_for=(Exception,),
    retry_backoff=True,
...
@shared_task(
    bind=True,
    name="tasks.generate_staff_daily_summary",
    max_retries=3,
    default_retry_delay=120,
    autoretry_for=(Exception,),
    retry_backoff=True,
    ...
    with _RedisLock(lock_key, ttl=3600) as acquired:
        ...
        with task_session() as session:
            ...
            for row in targets:
                ...
                """),
                    {"store_id": row.store_id, "yesterday": str(yesterday), "user_id": row.user_id},
                ).fetchone()
...
def credit_overdue_alerts(self):
    ...
    with task_session() as session:
        ...
        ledgers = (
            session.query(CreditLedger).filter(CreditLedger.balance > 0, CreditLedger.updated_at < overdue_date).all()
        )
...
def update_gst_transactions_task(self):
    ...
    with task_session() as session:
        ...
        for config in configs:
            ...
            existing_gst_txn_ids = (
                session.query(GSTTransaction.transaction_id)
                .filter_by(store_id=store_id, period=current_period)
                .subquery()
            )
    ...
    with _RedisLock(lock_key, ttl=1800) as acquired:
        ...
        with task_session() as session:
            memberships = session.execute(
                text("SELECT store_id FROM store_group_memberships WHERE group_id = :gid"), {"gid": group_id_str}
            ).fetchall()

            for m in memberships:
                daily_sum = session.execute(
                    text("""
                    SELECT revenue, profit, transaction_count
                    FROM daily_store_summary
                    WHERE store_id = :sid AND date = :tdate
                """),
                    {"sid": m.store_id, "tdate": str(today)},
                ).fetchone()
    ...
    with _RedisLock(lock_key, ttl=3600) as acquired:
        ...
        with task_session() as session:
            ...
            for alert in critical_alerts:
                ...
                surplus_candidates = session.execute(
                    text(
                    ...
                    {"pid": pid, "short_store": short_store},
                ).fetchall()
                ...
                suggested_qty = round((float(best_target.current_stock) - float(best_target.reorder_level)) * 0.5)
...
@shared_task(
    bind=True,
    name="tasks.run_weekly_pricing_analysis",
    max_retries=3,
    default_retry_delay=120,
    autoretry_for=(Exception,),
    retry_backoff=True,
        ...
        with task_session() as session:
            ...
            for store_row in stores:
                ...
                try:
                    ...
                    _log("run_weekly_pricing_analysis", store_id=sid, status="engine_error", error=str(exc))
                    continue
            ...
            _log("run_weekly_pricing_analysis", status="done", stores=len(stores), suggestions_inserted=total_inserted)
...
def process_ocr_job(self, job_id: str):
    ...
    with task_session() as session:
        # Load job
        job_row = session.execute(text("SELECT * FROM ocr_jobs WHERE id = :jid FOR UPDATE"), {"jid": job_id}).fetchone()
        ...
        # Mark PROCESSING
        session.execute(text("UPDATE ocr_jobs SET status = 'PROCESSING' WHERE id = :jid"), {"jid": job_id})
        session.commit()
    ...
    try:
        ...
        with task_session() as session:
            ...
            session.execute(
                text("UPDATE ocr_jobs SET raw_ocr_text = :text WHERE id = :jid"), {"text": raw_text, "jid": job_id}
            )
            ...
            # 3. Mark job REVIEW
            session.execute(text("UPDATE ocr_jobs SET status = 'REVIEW' WHERE id = :jid"), {"jid": job_id})
            session.commit()
