...
def send_alert():
    ...
    if not config or not config.phone_number_id or not config.access_token_encrypted:
        return format_response(False, error={"code": "WA_NOT_CONFIGURED", "message": "WhatsApp API is not configured or active for this store"}), 400
    ...
    if 'error' in resp:
        return format_response(False, error={"code": "WA_API_ERROR", "message": resp['error']}), 502

    return format_response(True, data={"message_id": log_entry.wa_message_id}), 200
...
def send_po():
    ...
    try:
        ...
        return format_response(False, error={"code": "VALIDATION_ERROR", "message": "Invalid PO ID format"}), 400

    po = db.session.query(PurchaseOrder).filter_by(id=po_uuid, store_id=store_id).first()
    if not po:
        return format_response(False, error={"code": "NOT_FOUND", "message": "PO not found"}), 404

    supplier = db.session.query(Supplier).filter_by(id=po.supplier_id).first()
    if not supplier or not supplier.phone:
        return format_response(False, error={"code": "NO_RECIPIENT", "message": "Supplier phone number not available"}), 400

    config = db.session.query(WhatsAppConfig).filter_by(store_id=store_id, is_active=True).first()
    if not config or not config.phone_number_id or not config.access_token_encrypted:
        return format_response(False, error={"code": "WA_NOT_CONFIGURED", "message": "WhatsApp is not configured"}), 400
