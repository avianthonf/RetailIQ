...
def format_po_message(po_id: str, db_session) -> str:
    ...
    for item in items:
        ...
        qty_str = f"{float(item.ordered_qty):g}" if hasattr(item, 'ordered_qty') and item.ordered_qty is not None else "0"
        lines.append(f"- {prod_name} x {qty_str} @ {price_str}")
        total_amount += (float(item.ordered_qty) if item.ordered_qty else 0.0) * (float(item.unit_price) if item.unit_price else 0.0)
