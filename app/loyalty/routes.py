...
from ..models import (
    LoyaltyProgram, CustomerLoyaltyAccount, LoyaltyTransaction,
...
def get_customer_loyalty(customer_id):
    ...
    txns = db.session.query(LoyaltyTransaction).filter_by(account_id=account.id).order_by(LoyaltyTransaction.created_at.desc()).limit(10).all()
    recent_transactions = []
    for t in txns:
...
def redeem_loyalty_points(customer_id):
    ...
    try:
        with db.session.begin_nested():
            ...
            account = db.session.query(CustomerLoyaltyAccount).with_for_update().filter_by(customer_id=customer_id, store_id=store_id).first()
        ...
        db.session.commit()
        return format_response(True, data={"message": "Points redeemed successfully", "remaining_points": float(account.total_points)}), 200
...
def get_customer_credit(customer_id):
    ...
    txns = db.session.query(CreditTransaction).filter_by(ledger_id=ledger.id).order_by(CreditTransaction.created_at.desc()).limit(10).all()
    recent_transactions = []
    for t in txns:
        recent_transactions.append({
            "type": t.type,
...
def repay_credit(customer_id):
    ...
    try:
        with db.session.begin_nested():
            ledger = db.session.query(CreditLedger).with_for_update().filter_by(customer_id=customer_id, store_id=store_id).first()
            if not ledger:
        ...
        db.session.commit()
        return format_response(True, data={"message": "Repayment successful", "remaining_balance": float(ledger.balance)}), 200
    except ValueError as e:
        db.session.rollback()
        return format_response(False, error={"code": "UNPROCESSABLE_ENTITY", "message": str(e)}), 422
    except Exception as e:
...
def loyalty_analytics():
    ...
    earned_this_month = db.session.query(func.sum(LoyaltyTransaction.points)).filter(
        LoyaltyTransaction.account_id.in_(accounts),
        LoyaltyTransaction.type == 'EARN',
        LoyaltyTransaction.created_at >= start_of_month
    ).scalar() or 0
    
    redeemed_this_month = db.session.query(func.sum(LoyaltyTransaction.points)).filter(
        LoyaltyTransaction.account_id.in_(accounts),
