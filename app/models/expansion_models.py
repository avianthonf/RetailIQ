...
class PaymentProvider(Base):
    """Registry of local payment providers/adapters per country."""

    __tablename__ = "payment_providers"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    code: Mapped[str] = mapped_column(String(32), unique=True, nullable=False)  # razorpay, stripe, mpesa
    name: Mapped[str] = mapped_column(String(128), nullable=False)
    country_code: Mapped[str] = mapped_column(String(2), ForeignKey("countries.code"), nullable=False)
    provider_type: Mapped[str] = mapped_column(String(32), nullable=False)  # PSP, BANK, MOBILE_MONEY
    supported_methods: Mapped[dict | None] = mapped_column(JSONB)  # ["UPI","CARD","NET_BANKING"]
    ...
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
    config: Mapped[dict | None] = mapped_column(JSONB)  # provider-specific config (encrypted in practice)


class StorePaymentMethod(Base):
    """Links stores to enabled payment methods."""

    __tablename__ = "store_payment_methods"

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    store_id: Mapped[int] = mapped_column(Integer, ForeignKey("stores.store_id"), nullable=False)
    provider_id: Mapped[int] = mapped_column(Integer, ForeignKey("payment_providers.id"), nullable=False)
    merchant_account_id: Mapped[str | None] = mapped_column(String(128))  # store's account at provider
    is_enabled: Mapped[bool] = mapped_column(Boolean, default=True)
    onboarding_status: Mapped[str] = mapped_column(String(16), default="PENDING")
...
class PaymentRecord(Base):
    """Records individual payment attempts through local providers."""
    ...
    store_id: Mapped[int] = mapped_column(Integer, ForeignKey("stores.store_id"), nullable=False)
    provider_id: Mapped[int] = mapped_column(Integer, ForeignKey("payment_providers.id"), nullable=False)
    payment_method: Mapped[str] = mapped_column(String(32), nullable=False)  # UPI, PIX, M-PESA, SEPA
    ...
    __table_args__ = (
        Index("idx_payment_records_store_status", "store_id", "status"),
        Index("idx_payment_records_provider_ref", "provider_ref_id"),
    )
