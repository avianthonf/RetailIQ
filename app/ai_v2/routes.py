...
def forecast_v2():
    ...
    return format_response(data=result), 200


@ai_v2_bp.route("/pricing/optimize", methods=["POST"])
@require_auth
def optimize_pricing_v2():
    data = request.json or {}
    product_ids = data.get("product_ids", [])
    store_id = g.current_user["store_id"]
    objective = data.get("objective", "profit")

    if not product_ids:
        return format_response(False, error={"code": "MISSING_PARAM", "message": "product_ids is required"}), 400

    from ..pricing.engine import generate_optimal_price

    recommendations = generate_optimal_price(store_id, product_ids, db.session, objective=objective)
    return format_response(data=recommendations), 200
...
def shelf_scan_v2():
    ...
    if not image_url:
        return format_response(False, error={"code": "MISSING_PARAM", "message": "image_url is required"}), 400

    from ..vision.shelf import process_shelf_scan

    analysis = process_shelf_scan(image_url)
    return format_response(data={"analysis": analysis}), 200
